...
typedef struct
{
  ...
  uint8_t Advertising_additionnal_timer_Id;
  /* USER CODE END PTD_1 */
}BleApplicationContext_t;

/* USER CODE BEGIN PTD */
#define SHOT_ADV_TIMEOUT            (1*1000*1000/CFG_TS_TICK_VAL) /**< 1s */
/* USER CODE END PTD */
...
/* USER CODE BEGIN PD */
#undef  CFG_STATIC_RANDOM_ADDRESS
/* USER CODE END PD */

/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
static void update_adv_shot_data(void);

// D�finir une fonction �quivalente � strnlen
 ...
 */
#if (P2P_SERVER1 != 0)
//static const char a_LocalName[] = { AD_TYPE_COMPLETE_LOCAL_NAME ,'C','T','O','R','1','0','W'};
char a_LocalName[SIZE_OF_SSID+1]; //add LOCAL NAME PREFIX
uint8_t a_LocalName_length = SIZE_OF_SSID+1;
...
void APP_BLE_Init(void)
{
  ...
  /* USER CODE BEGIN APP_BLE_Init_1 */
	...
	// Calculer la longueur r�elle de la cha�ne SSID
  size_t ssid_length = my_strnlen(Eeprom_name_Config.SSID, SIZE_OF_SSID);
  // Copiez le SSID en vous assurant qu'il se termine par '\0'
  strncpy(&a_LocalName[1], Eeprom_name_Config.SSID, ssid_length);
  ...
  /* USER CODE BEGIN APP_BLE_Init_3 */
	//UTIL_SEQ_RegTask( 1<< CFG_TASK_SHOT_ADV_ID, UTIL_SEQ_RFU, update_adv_shot_data );
	UTIL_SEQ_RegTask( 1<< CFG_TASK_SHOT_ADV_ID, UTIL_SEQ_RFU, additionnal_adv );
	UTIL_SEQ_RegTask(1<<CFG_TASK_SHOT_ADV_CANCEL_ID, UTIL_SEQ_RFU, additionnal_adv_Cancel);
	/**
   * Create timer to handle the Shot Advertising Stop
   */
  HW_TS_Create(CFG_TIM_PROC_ID_ISR, &(BleApplicationContext.Advertising_additionnal_timer_Id), hw_ts_SingleShot, additionnal_adv_Cancel);
...
#define ADV_CH_ALL 0x07 // Cette d�finition signifie que tous les canaux publicitaires (37, 38 et 39) sont utilis�s.

// Fonction pour mettre � jour la valeur de t�l�m�trie envoy�e
void update_adv_shot_data(void) {
	uint8_t initial_adv_shot_data = 0x78;
	
// D�finition des donn�es du beacon Eddystone TLM
/*uint8_t beacon_data[18] = {
    17,                          //< Length.
    AD_TYPE_SERVICE_DATA,        //< Service Data data type value.
    0xAA, 0xFE,                  //< 16-bit Eddystone UUID.
    0x20,                        //< TLM frame type.
    0x00,                        //< Version.
    0x00, 0x00,                  //< Battery Voltage (2 bytes).
    0x00, telemetry_data,                  //< Beacon Temperature (2 bytes).
    0x00, 0x00, 0x00, 0x00,      //< Advertising PDU count (4 bytes).
    0x00, 0x00, 0x00, 0x00,      //< Time since power-on or reboot (4 bytes).
};*/
/*uint8_t beacon_data[17] =
  {
    ...
    CFG_TX_POWER,                                       //< URL Scheme Prefix is http://www.
    ...
    AD_TYPE_SERVICE_DATA,                               //< Service Data data type value.
    0xAA, 0xFE,                                         //< 16-bit Eddystone UUID.
    0x30,                         // Frame Type (Eddystone-EID)
    CFG_TX_POWER,                         // TX Power
    P2P_Server_App_Context.ShotToNotification[0], 
		P2P_Server_App_Context.ShotToNotification[1],
		P2P_Server_App_Context.ShotToNotification[2],
		P2P_Server_App_Context.ShotToNotification[3],
		P2P_Server_App_Context.ShotToNotification[4],
		P2P_Server_App_Context.ShotToNotification[5],
		P2P_Server_App_Context.ShotToNotification[6],
		P2P_Server_App_Context.ShotToNotification[7],
		P2P_Server_App_Context.ShotToNotification[8],
		P2P_Server_App_Context.ShotToNotification[9],
		P2P_Server_App_Context.ShotToNotification[10],
		P2P_Server_App_Context.ShotToNotification[11],
		P2P_Server_App_Context.ShotToNotification[12]
};
    // Configurer les donn�es de la balise suppl�mentaire
    tBleStatus ret = aci_gap_additional_beacon_set_data(
        sizeof(beacon_data), // Taille des donn�es de la balise
        beacon_data          // Pointeur vers les donn�es de la balise
    );
...
// MAC address
    ...
    own_address[0] = (uint8_t)(mac_address >> 0);
    own_address[1] = (uint8_t)(mac_address >> 8);
    own_address[2] = (uint8_t)(mac_address >> 16);
    own_address[3] = (uint8_t)(mac_address >> 24);
    own_address[4] = (uint8_t)(mac_address >> 32);
    own_address[5] = (uint8_t)(mac_address >> 40);
...
void additionnal_adv_Cancel(void)
{
  ...
  HW_TS_Stop(BleApplicationContext.Advertising_additionnal_timer_Id);
	...
	if (ret != BLE_STATUS_SUCCESS)
  {
     APP_DBG_MSG("** STOP ACI_GAP_ADDITIONNAL_BEACON_ADVERTISING **  Failed \r\n\r");
  }
  else
  {
...
static void Adv_Cancel_Req(void)
{
  ...
  UTIL_SEQ_SetTask(1 << CFG_TASK_ADV_CANCEL_ID, CFG_SCH_PRIO_4);
...
static void Adv_additionnal_Cancel_Req(void)
{
  /* USER CODE BEGIN Adv_Cancel_Req_1 */

  /* USER CODE END Adv_Cancel_Req_1 */

  UTIL_SEQ_SetTask(1 << CFG_TASK_SHOT_ADV_CANCEL_ID, CFG_SCH_PRIO_4);
